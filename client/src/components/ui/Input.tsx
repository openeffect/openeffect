...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, error, ...props }, ref) => {
    if (type === 'number') {
      return <NumberInput ref={ref} className={className} error={error} {...props} />
    }
...
const NumberInput = forwardRef<HTMLInputElement, Omit<InputHTMLAttributes<HTMLInputElement>, 'type'> & { error?: boolean }>(
  ({ className, step, min, max, onChange, error, ...props }, ref) => {
    ...
    return (
      <div className={cn(
        'flex items-center rounded-lg border bg-muted transition-colors hover:border-foreground/15 [&:has(:focus-visible)]:border-ring',
        error && 'border-destructive hover:border-destructive [&:has(:focus-visible)]:border-destructive',
        className,
      )}>
        <button
          type="button"
