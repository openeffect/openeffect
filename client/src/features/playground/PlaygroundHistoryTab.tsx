...
import { loadPlaygroundHistory } from '@/store/actions/historyActions'
...
export function PlaygroundHistoryTab() {
  ...
  const handleIntersect = useCallback((entries: IntersectionObserverEntry[]) => {
    ...
    if (
      ...
      s.history.playgroundItems.length < s.history.playgroundTotal &&
      s.history.playgroundStatus !== 'loading'
    ) {
      loadPlaygroundHistory(s.history.playgroundItems.length)
    }
  }, [])
