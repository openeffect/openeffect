 ...
 * URL patterns:
 *   /                            → gallery
 *   /?type=X&source=Y            → gallery with filters applied
 ...
 *
 * The `type` / `source` query params are context: they live on whichever
...
export type Filters = { type: string; source: EffectSource; search: string }
...
export function parseFilters(search: string): Filters {
  ...
  return { type, source, search: q }
}

export function parseRoute(url?: string): ParsedRoute {
  if (!url && typeof window === 'undefined') return { page: 'gallery', type: 'all', source: 'all', search: '' }
 ...
 *  type/source context travels through every navigation for free. Caller's
...
function buildQuery(query?: Record<string, string | undefined>): URLSearchParams {
  ...
  for (const key of ['type', 'source', 'search'] as const) {
  ...
  if (query) {
    for (const [key, value] of Object.entries(query)) {
      ...
      if ((key === 'type' || key === 'source') && value === 'all') continue
      out.set(key, value)
    }
  }
  return out
...
export function initRouteListener(
  ...
  window.addEventListener('popstate', () => {
    ...
    if (route.page === 'effect') {
      onEffect(route.effectId, route.runId, { type: route.type, source: route.source, search: route.search })
    } else if (route.page === 'edit') {
      onEdit(route.effectId, { type: route.type, source: route.source, search: route.search })
    } else if (route.page === 'playground') {
      onPlayground(route.runId, { type: route.type, source: route.source, search: route.search })
    } else if (route.page === 'newEffect') {
      onNewEffect()
    } else {
      onGallery({ type: route.type, source: route.source, search: route.search })
    }
  })
}
