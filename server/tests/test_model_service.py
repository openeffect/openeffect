...
class TestGetCompatibleModelIds:
    ...
    def test_known_models_present(self):
        ...
        # WAN 2.2 wires both roles on fal → in both lists
        assert "wan-2.2" in start_only
        assert "wan-2.2" in both

        # Kling V3 wires both roles on fal → in both lists
...
class TestGetProvider:
    def test_returns_full_provider_dict(self):
        provider = get_provider("wan-2.2", "fal")
        assert provider is not None
        assert provider["endpoint"] == "fal-ai/wan/v2.2-a14b/image-to-video"
    ...
    def test_unknown_provider_returns_none(self):
        assert get_provider("kling-v3", "replicate") is None


class TestProviderMaps:
    def test_wan22_fal_image_inputs(self):
    ...
    def test_kling_v3_fal_image_inputs(self):
        provider = get_provider("kling-v3", "fal")
...
class TestModelImageInputs:
    ...
    def test_start_frame_is_required(self):
        for model_id in ("wan-2.2", "kling-v3", "pixverse-v6"):
...
class TestGenerateAudio:
    def test_models_with_audio(self):
        # Kling V3 and PixVerse V6 expose generate_audio as a canonical param
...
class TestCanonicalToWire:
    ...
    def test_unknown_canonicals_are_dropped(self):
        ...
        (transform outputs)."""
        out = self._apply("kling-v3", {"duration": 5, "unknown_knob": 42})
        assert out == {"duration": 5, "unknown_knob": 42}

    def test_image_role_renames_to_wire_key(self):
        out = self._apply("wan-2.2", {
            "start_frame": "https://x/y.jpg", "duration": 5,
        })
        # wan's transform consumes `duration` (seconds) and emits both
        # `num_frames` and `frames_per_second` as wire keys (derived, not
        # canonical). start_frame canonical → wire `image_url`.
        assert out["image_url"] == "https://x/y.jpg"
        assert out["num_frames"] == 5 * 16
        assert out["frames_per_second"] == 16

    def test_wan22_transform_clamps_below_fal_minimum(self):
        """A manifest that locks duration below fal's floor (17 frames at
        16fps ≈ 2s) shouldn't ship a bad request. The transform clamps."""
        out = self._apply("wan-2.2", {"duration": 1})
        assert out["num_frames"] == 17       # clamped up from 1 × 16 = 16
        assert out["frames_per_second"] == 16

    def test_wan22_transform_clamps_above_fal_maximum(self):
        out = self._apply("wan-2.2", {"duration": 99})
        assert out["num_frames"] == 161      # clamped down from 99 × 16
        assert out["frames_per_second"] == 16
    ...
    def test_self_named_wire_passes_through(self):
        ...
        out = self._apply("kling-v3", {"duration": 5})
    ...
    def test_missing_provider_transform_ok(self):
        ...
        out = self._apply("kling-v3", {"guidance_scale": 0.7})
    ...
    def test_unsupported_canonical_dropped_silently(self):
        ...
        (e.g. `style` on wan-2.2) gets silently dropped."""
        # style is a pixverse-v6 canonical; not declared on wan-2.2.
