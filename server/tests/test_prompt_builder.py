...
def make_manifest(**overrides) -> EffectManifest:
    ...
    defaults = {
        ...
        "inputs": {
            "image": InputFieldSchema(type="image", required=True, label="Photo", accept=["image/jpeg"], max_size_mb=10),
            "prompt": InputFieldSchema(type="text", required=False, label="Prompt", placeholder="Describe...", max_length=300, multiline=False),
        },
        "output": OutputConfig(
...
class TestBuildPrompt:
    def test_basic_prompt_substitution(self):
        ...
        result = PromptBuilder.build_prompt(manifest, "fal-ai/wan-2.2", {"prompt": "epic sunset"})
        assert "epic sunset" in result
        assert "{prompt}" not in result

    def test_empty_prompt_no_double_spaces(self):
        manifest = make_manifest()
        result = PromptBuilder.build_prompt(manifest, "fal-ai/wan-2.2", {"prompt": ""})
        assert "  " not in result
        assert "{prompt}" not in result

    def test_missing_prompt_field(self):
        manifest = make_manifest()
        result = PromptBuilder.build_prompt(manifest, "fal-ai/wan-2.2", {})
        assert "  " not in result
        assert "{prompt}" not in result
    ...
    def test_text_field_substitution(self):
        manifest = make_manifest(
            inputs={
                "image": InputFieldSchema(type="image", required=True, label="Photo", accept=["image/jpeg"], max_size_mb=10),
                "mood": InputFieldSchema(type="text", required=False, label="Mood", max_length=100, multiline=False),
            },
            generation=GenerationConfig(
        ...
        result = PromptBuilder.build_prompt(manifest, "fal-ai/wan-2.2", {"mood": "dramatic"})
    ...
    def test_select_field_uses_label_not_value(self):
        ...
        result = PromptBuilder.build_prompt(manifest, "fal-ai/wan-2.2", {"style": "liquid"})
    ...
    def test_model_override_uses_different_template(self):
        manifest = make_manifest(
            generation=GenerationConfig(
                ...
                parameters={},
                model_overrides={
                    "fal-ai/kling-v3": ModelOverride(prompt_template="Kling template. {prompt}"),
                },
            ),
        )
        result_wan = PromptBuilder.build_prompt(manifest, "fal-ai/wan-2.2", {"prompt": "test"})
        result_kling = PromptBuilder.build_prompt(manifest, "fal-ai/kling-v3", {"prompt": "test"})
...
class TestBuildParams:
    def test_unknown_key_in_defaults_filtered(self):
        ...
        result = PromptBuilder.build_params(manifest, "fal-ai/wan-2.2")
        assert "guidance_scale" in result
        assert "unknown_param" not in result

    def test_unknown_key_in_user_params_filtered(self):
        manifest = make_manifest()
        result = PromptBuilder.build_params(manifest, "fal-ai/wan-2.2", {"guidance_scale": 8.0, "fake_key": 99})
    ...
    def test_model_override_merges_on_top(self):
        manifest = make_manifest(
            generation=GenerationConfig(
                ...
                model_overrides={
                    "fal-ai/kling-v3": ModelOverride(parameters={"guidance_scale": 9.0}),
                },
            ),
        )
        result = PromptBuilder.build_params(manifest, "fal-ai/kling-v3")
    ...
    def test_user_params_merge_on_top_of_override(self):
        manifest = make_manifest(
            generation=GenerationConfig(
                prompt_template="test",
                supported_models=["fal-ai/kling-v3"],
                default_model="fal-ai/kling-v3",
                parameters={"guidance_scale": 7.5},
                model_overrides={
                    "fal-ai/kling-v3": ModelOverride(parameters={"guidance_scale": 9.0}),
                },
        ...
        result = PromptBuilder.build_params(manifest, "fal-ai/kling-v3", {"guidance_scale": 12.0})
        assert result["guidance_scale"] == 12.0

    def test_empty_user_params(self):
        manifest = make_manifest()
        result = PromptBuilder.build_params(manifest, "fal-ai/wan-2.2", None)
    ...
    def test_prompt_inputs_never_in_params(self):
        ...
        manifest = make_manifest(
            inputs={
                "style": InputFieldSchema(
                    type="select", required=False, label="Style", default="particles",
                    options=[SelectOption(value="particles", label="Particles")],
                ),
            },
        )
        result = PromptBuilder.build_params(manifest, "fal-ai/wan-2.2", {"style": "particles"})
